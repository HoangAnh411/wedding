...
export default function WeddingDetailClient({ wedding }: { wedding: WeddingDetail }) {
  ...
  const handleSendAllEmail = async () => {
    if (!confirm(dict.admin.weddingDetail.confirmSendAll)) return;
    ...
    try {
      ...
      setEmailResult({
        sent: 0,
        failed: 1,
        results: [{ name: "", success: false, message: dict.admin.weddingDetail.connError }],
      });
    } finally {
  ...
  const copyToClipboard = () => {
    ...
    alert(dict.admin.weddingDetail.copied);
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white p-8 md:p-10 shadow-sm border border-gray-200">
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p className="text-gray-500 font-semibold tracking-wider text-xs uppercase mb-2">{dict.admin.weddingDetail.manageWedding}</p>
              ...
              <div className="flex flex-wrap gap-4 text-sm font-medium text-gray-600">
                <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-4 py-2 border border-gray-100">
                  ...
                  {wedding.weddingDate ? new Date(wedding.weddingDate).toLocaleDateString("vi-VN") : dict.admin.weddingDetail.noDate}
                </div>
                {wedding.venueName && (
                  <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-4 py-2 border border-gray-100">
                    <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-4 py-2 border border-gray-100">
                  ...
                  {wedding.viewCount || 0} {dict.admin.weddingDetail.views}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                ...
                {sendingEmail ? dict.admin.weddingDetail.sending : dict.admin.weddingDetail.sendInvite}
              </button>
            </div>
          </div>
        </div>
      ...
      {emailResult && (
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="font-medium text-gray-900">{dict.admin.weddingDetail.emailResult}</p>
          <div className="mt-2 flex gap-4 text-sm">
            <span className="text-green-600 font-medium">{dict.admin.weddingDetail.success} {emailResult.sent}</span>
            <span className="text-red-600 font-medium">{dict.admin.weddingDetail.failed} {emailResult.failed}</span>
          </div>
            ...
            <div className="mt-4 max-h-40 overflow-y-auto space-y-2 border-t border-gray-100 pt-3">
              {emailResult.results.filter((r) => !r.success).map((r, i) => (
                <p key={i} className="text-sm text-red-500">
                  <span className="font-medium">{r.name || dict.admin.weddingDetail.guest}:</span> {r.message}
                </p>
              ))}
            </div>
          )}
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <StatsCard 
          title={dict.admin.weddingDetail.stats.totalGuests} 
        ...
        />
        <StatsCard 
          title={dict.admin.weddingDetail.stats.confirmed} 
        ...
        />
        <StatsCard 
          title={dict.admin.weddingDetail.stats.tables} 
        ...
        />
        <StatsCard 
          title={dict.admin.weddingDetail.stats.tasks} 
        ...
        />
        <StatsCard 
          title={dict.admin.weddingDetail.stats.gifts} 
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 md:p-8 shadow-sm">
        <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
          {dict.admin.weddingDetail.quickAccess}
        </h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <QuickLink href={`/admin/weddings/${wedding.id}/guests`} icon="👥" label={dict.admin.sidebar.guests} />
          <QuickLink href={`/admin/weddings/${wedding.id}/tables`} icon="🪑" label={dict.admin.sidebar.tables} />
          <QuickLink href={`/admin/weddings/${wedding.id}/budget`} icon="💰" label={dict.admin.sidebar.budget} />
          <QuickLink href={`/admin/weddings/${wedding.id}/checklist`} icon="📋" label={dict.admin.sidebar.checklist} />
          <QuickLink href={`/admin/weddings/${wedding.id}/gallery`} icon="🖼️" label={dict.admin.sidebar.gallery} />
          <QuickLink href={`/admin/weddings/${wedding.id}/music`} icon="🎵" label={dict.admin.sidebar.music} />
          <QuickLink href={`/admin/weddings/${wedding.id}/wishes`} icon="💌" label={dict.admin.sidebar.wishes} />
          <QuickLink href={`/admin/weddings/${wedding.id}/money-gifts`} icon="🎁" label={dict.admin.sidebar.moneyGifts} />
          <QuickLink href={`/admin/weddings/${wedding.id}/checkin`} icon="📷" label={dict.admin.sidebar.checkin} />
          <QuickLink href={`/admin/weddings/${wedding.id}/settings`} icon="⚙️" label={dict.admin.sidebar.settings} />
        </div>
      </div>
    </div>
  );
