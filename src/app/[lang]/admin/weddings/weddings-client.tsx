...
export default function WeddingsClient({
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(dict.admin.weddings.error);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{dict.admin.weddings.title}</h1>
          <p className="mt-1 text-sm text-gray-500">{dict.admin.weddings.subtitle}</p>
        </div>
        <button
      ...
      {weddings.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-gray-200 bg-white p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900">{dict.admin.weddings.noWedding}</h3>
          <p className="mt-2 text-sm text-gray-500">{dict.admin.weddings.noWeddingDesc}</p>
          <button
            onClick={() => setShowCreate(true)}
          ...
          {weddings.map((w) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-gray-500">
                {w.weddingDate ? new Date(w.weddingDate).toLocaleDateString("vi-VN") : dict.admin.weddings.noDate}
              </p>
              {w.venueName && <p className="mt-1 text-xs text-gray-400">{w.venueName}</p>}
              <div className="mt-4 flex items-center gap-4 text-xs text-gray-500">
                <span>👥 {w.guestCount} {dict.admin.weddings.guests}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="mx-4 w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-gray-900">{dict.admin.weddings.createModalTitle}</h2>
            <p className="mt-1 text-sm text-gray-500">{dict.admin.weddings.createModalDesc}</p>
            {error && <div className="mt-3 rounded-lg bg-red-50 p-3 text-sm text-red-600">{error}</div>}
            <form className="mt-6 space-y-4" onSubmit={handleCreate}>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">{dict.admin.weddings.groomName}</label>
                  <input type="text" required value={form.groomName}
                    ...
                    placeholder={dict.admin.weddings.groomPlaceholder} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">{dict.admin.weddings.brideName}</label>
                  <input type="text" required value={form.brideName}
                    onChange={(e) => setForm({ ...form, brideName: e.target.value })}
                    className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500"
                    placeholder={dict.admin.weddings.bridePlaceholder} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">{dict.admin.weddings.weddingDate}</label>
                <input type="date" value={form.weddingDate}
                  onChange={(e) => setForm({ ...form, weddingDate: e.target.value })}
                  className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">{dict.admin.weddings.venueName}</label>
                <input type="text" value={form.venueName}
                  onChange={(e) => setForm({ ...form, venueName: e.target.value })}
                  className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500"
                  placeholder={dict.admin.weddings.venuePlaceholder} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">{dict.admin.weddings.clientEmail}</label>
                <input type="email" value={form.clientEmail}
                  onChange={(e) => setForm({ ...form, clientEmail: e.target.value })}
                  className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500"
                  placeholder="email@example.com" />
                <p className="mt-1 text-xs text-gray-500">{dict.admin.weddings.clientEmailDesc}</p>
              </div>
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? dict.admin.weddings.creating : dict.admin.weddings.createButton}
                </button>
              </div>
            </form>
          </div>
