...
export default function StaffClient({ initialStaff }: { initialStaff: StaffMember[] }) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || dict.admin.staff.genericError);
      }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(dict.admin.staff.confirmDelete)) return;
    ...
    <div className="space-y-6">
      {!isAdding ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="mb-4 text-lg font-medium">{dict.admin.staff.addStaffTitle}</h3>
          {error && <p className="mb-4 text-sm text-red-600">{error}</p>}
          <form onSubmit={handleAdd} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">{dict.admin.staff.name}</label>
              <input
                type="text"
                required
                value={name}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">{dict.admin.staff.email}</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">{dict.admin.staff.password}</label>
              <input
                type="password"
                required
                value={password}
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? dict.admin.staff.saving : dict.admin.staff.save}
              </button>
              <button
                type="button"
                onClick={() => {
          ...
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="relative px-6 py-3">
                <span className="sr-only">{dict.admin.staff.actions}</span>
              </th>
            </tr>
          </thead>
