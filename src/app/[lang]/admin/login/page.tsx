...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        setError(dict.admin.login.errorCredentials);
        setLoading(false);
        return;
      }
      ...
      setError(dict.admin.login.errorGeneric);
      setLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-rose-50 to-white px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? dict.admin.login.loggingIn : dict.admin.login.loginButton}
          </button>
        </form>
