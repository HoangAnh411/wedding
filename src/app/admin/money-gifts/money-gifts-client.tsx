...
export default function MoneyGiftsClient({
  ...
  const [form, setForm] = useState({ weddingId: weddings[0]?.id || "", guestName: "", amount: "", paymentMethod: "Tiền mặt" });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setShowAdd(false);
        setForm({ weddingId: weddings[0]?.id || "", guestName: "", amount: "", paymentMethod: "Tiền mặt" });
      }
    } finally {
      setLoading(false);
    }
